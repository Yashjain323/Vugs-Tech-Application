...
public class sellerprofilemanage extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sellerprofilemanage);
        ImageView imageView;
        DatabaseReference dsbref = FirebaseDatabase.getInstance().getReference("SellerProfie");
        imageView = (ImageView)this.findViewById(R.id.imageViewseller);
        Button photoButton = (Button) this.findViewById(R.id.button30seller);
        ...
        email.setText(emailIDInput.getText().toString().trim(), TextView.BufferType.EDITABLE);
        name.setText(fNameInput + " " + lNameInput,TextView.BufferType.EDITABLE);
        address.setText(useraddressInput.getText().toString().trim(), TextView.BufferType.EDITABLE);
        ...
        img2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                dsbref.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child("fname").setValue(name.getText());
            }
        });
        img3.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                dsbref.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child("emailId").setValue(email.getText());
            }
        });
        img5.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                dsbref.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child("address").setValue(address.getText());
            }
        });
